#!/bin/bash
# Standard load test wrapper (4+ minutes with ramp)
set -e
...
JSON_REPORT="test-results/artillery/standard_${TIMESTAMP}.json"
HTML_REPORT="test-results/artillery/standard_${TIMESTAMP}.html"

# Run standard load test with proper variables and save results
